...
def measureData():
    ...
    select 区域,[社保局（月结）],证件号码,姓名,医院名称,医疗类别,出院日期,入院诊断,ICD编码,出院诊断,就诊病种代码,医疗费用总额,起付标准,范围内费用,基本医疗统筹支付,[大病（补充）医疗统筹支付],结算类别,级别,建档立卡,特困供养 into #建模数据
    from [dbo].[汇总]
    where left(出院日期,4)=@年份 
    
    alter table #建模数据 add 诊断 varchar(255)
